...
// SITE.cpp : 응용 프로그램에 대한 클래스 동작을 정의합니다.
//
...
BEGIN_MESSAGE_MAP(CSITEApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CSITEApp::OnAppAbout)
...
BOOL CSITEApp::InitInstance()
{
	int nMSGresult = 0;
	nMSGresult = AfxMessageBox(APPSTART_MSG, MB_YESNO|MB_ICONINFORMATION);
	if ( nMSGresult != IDYES ) return FALSE;
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CSITEDoc),
		RUNTIME_CLASS(CMainFrame),       // 주 SDI 프레임 창입니다.
		RUNTIME_CLASS(CSITEView));
	if (!pDocTemplate)
		return FALSE;
